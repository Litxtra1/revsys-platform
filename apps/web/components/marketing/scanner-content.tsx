...
function ScannerTheater() {
  ...
  const currentStep = SCAN_STEPS[active] ?? SCAN_STEPS[0];

  useEffect(() => {
    setLogCursor(0);
    const id = setInterval(() => setLogCursor((c) => Math.min(c + 1, currentStep.logs.length)), 500);
    return () => clearInterval(id);
  }, [currentStep]);
...
function LockGlyph() {
  return (
    <span className="grid h-9 w-9 place-items-center rounded-full bg-revenue/10 text-revenue">
...
function RevenueReport({ report }: { report?: MockRevenueReport | null }) {
  ...
  const topLeaks = report
    ...
    : [
        { title: "Checkout shipping delay", detail: "2.4s median on mobile — highest-impact single fix", sev: "Critical", sevTone: "critical" as const, conf: "97%", amt: "$18,200", category: "Checkout" },
        { title: "Missing product evidence", detail: "12 top-selling PDPs lack sizing, reviews or return info", sev: "High", sevTone: "warning" as const, conf: "92%", amt: "$12,400", category: "Product Pages" },
        { title: "Unmet demand on sold-out SKUs", detail: "Search intent exceeds inventory in 3 categories", sev: "Medium", sevTone: "warning" as const, conf: "89%", amt: "$7,900", category: "Conversion" },
        { title: "Discount stacking overlap", detail: "Accidental combinations quietly erode margin", sev: "Medium", sevTone: "warning" as const, conf: "85%", amt: "$4,300", category: "Pricing" },
      ];
  const firstLeak = topLeaks[0];

  // Teaser gating: unlock the top 2 highest-priority categories in full,
  // keep the rest visible-but-blurred behind a signup CTA.
  const categoryOrder = [...new Set(topLeaks.map((leak) => leak.category))];
  const unlockedCategories = categoryOrder.slice(0, 2);
  const unlockedLeaks = topLeaks.filter((leak) => unlockedCategories.includes(leak.category));
  const lockedCategories = categoryOrder.slice(2);
          ...
          <div className="grid gap-10 border-b border-hairline px-8 py-10 md:grid-cols-[1.15fr_1fr] md:px-10">
            ...
            <div className="rounded-2xl border border-hairline bg-surface p-6">
              ...
              <div className="mt-5 flex items-center justify-between border-t border-hairline pt-4 text-[12px]">
                ...
                <span className="font-mono font-semibold text-revenue">+{firstLeak?.amt ?? "$0"} / mo</span>
              </div>
            </div>
          </div>
          ...
          <div className="border-b border-hairline px-8 py-10 md:px-10">
            <div className="mb-6 flex items-baseline justify-between">
              ...
              <span className="font-mono text-[11px] text-ink-4">
                {topLeaks.length} leaks across {categoryOrder.length}{" "}
                {categoryOrder.length === 1 ? "category" : "categories"}
              </span>
            </div>
            ...
            {unlockedLeaks.map((row) => (
              ...
              >
                <div>
                  <div className="font-mono text-[9.5px] uppercase tracking-[0.14em] text-growth">
                    {row.category}
                  </div>
                  <div className="mt-0.5 font-medium text-ink-1">{row.title}</div>
            ...
            {lockedCategories.length > 0 ? (
              <div className="relative mt-6 overflow-hidden rounded-2xl border border-hairline">
                <div aria-hidden className="select-none space-y-3 p-6 blur-[3px]">
                  {lockedCategories.map((category) => (
                    <div key={category} className="flex items-center justify-between text-[14px]">
                      <span className="font-medium text-ink-1">{category}</span>
                      <span className="font-display font-semibold text-revenue">$#,###</span>
                    </div>
                  ))}
                </div>
                <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 bg-card/75 px-6 text-center backdrop-blur-[1px]">
                  <LockGlyph />
                  <p className="max-w-xs text-[13.5px] leading-relaxed text-ink-2">
                    {lockedCategories.length} more {lockedCategories.length === 1 ? "category" : "categories"} of
                    findings are ready in your full Revenue Command Center.
                  </p>
                  <Link
                    href="/signup"
                    className="inline-flex items-center gap-2 rounded-full bg-revenue px-5 py-2.5 text-[13px] font-semibold text-revenue-foreground transition-all hover:brightness-110"
                  >
                    Unlock your full Command Center →
                  </Link>
                </div>
              </div>
            ) : null}
  ...
  return (
    <section className="px-6 py-24">
      <div className="mx-auto flex max-w-4xl flex-col items-center gap-6 text-center">
        ...
        <Link href="/pricing" className="text-[13px] font-medium text-ink-3 hover:text-ink-1">
          Or see pricing →
        </Link>
      </div>
    </section>
