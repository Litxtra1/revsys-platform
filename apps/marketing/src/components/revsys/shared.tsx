...
import { scanStoreFn } from "../../lib/scan.server";

/* ============================================================
   Revsys AI · Shared chrome + primitives (Book XIII)
   Extracted from the original single-page build so every route
   inherits the same navigation, footer, motion utilities and
   consulting-grade design vocabulary.
   ============================================================ */
    ...
    <div>
      <form
        onSubmit={(e) => {
          ...
          setScanning(true);
          setError(null);
          scanStoreFn({ data: { storeUrl: value } })
            .then((result) => {
              if (result.storeUrl) {
                navigate({ to: "/scanner", search: { url: result.storeUrl } });
                return;
              }
              setError(result.error ?? "We couldn't scan that store.");
              setScanning(false);
            })
            .catch(() => {
      ...
      >
        ...
        <button
          ...
          disabled={scanning}
        >
          {scanning ? "Scanning…" : "Start free scan"}
        ...
        <div className="hidden items-center gap-8 md:flex">
          {NAV_LINKS.map((l) => (
            <Link
              ...
              className="group relative text-[13px] font-medium text-ink-3 transition-colors hover:text-ink-1"
            >
              {l.label}
    ...
    <div>
      ...
      <ul className="mt-5 space-y-3">
        {links.map(([label, href]) => (
          <li key={label}>
            <Link to={href} className="text-[13.5px] text-ink-2 hover:text-revenue">
              {label}
            </Link>
          </li>
        ))}
